...
public class VerdantFangsEffect : SkillEffectSO
{
    public override string ApplyEffect(CombatantInstance user, CombatantInstance target, SkillSO skill)
    {
        ...
        {
            int baseBonusDmg = Mathf.RoundToInt(user.GetEffectiveHealPower(user.CurrentHealPower));

            target.AddEffect(new Effect(EffectType.HealingWeaken, 1, true, baseBonusDmg));

            // Both heal
            user.HealCombatant(baseBonusDmg);
            target.HealCombatant(Mathf.RoundToInt(target.GetEffectiveHealPower(target.CurrentHealPower)));
